...
import Clapi.Types.Path
  ( Path(..), pattern Root, pattern (:/), fromText, toText
  , isChildOf, joinSegs, AbsRel(..))
...
pathSpec _ = do
    describe "Quasiquoter" $
      it "Produces expected path" $
        [pathq|/oi/mate|] `shouldBe` Path @ar [[segq|oi|], [segq|mate|]]
    describe ":/" $ do
  ...
  where
    shouldBeGoodPath t p = (fromText t :: CanFail (Path ar)) `shouldBe` Right p
    shouldBeBadPath t = (fromText t :: CanFail (Path ar)) `shouldSatisfy` isLeft
